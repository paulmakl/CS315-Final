 ...
 * Code from https://github.com/frenchtoast747/WebGL-Obj-Loader
 * by frenchtoast747
 */
...
obj_loader.Mesh = function( objectData ){
    ...
    */
    ...
    for( var i=0; i<lines.length; i++ ){
      line = lines[i].split(/ +/); //regex split
      // if this is a vertex
      if(line[0] == "v"){
        verts.push( line[ 1 ] );
        verts.push( line[ 2 ] );
      ...
      // if this is a texture
        ...
        textures.push( line[ 2 ] );
      }
      // if this is a face
      else if(line[0] == 'f'){
        line = line.slice(1); //trim off the f
        var quad = false;
        for(var j=0; j<line.length; j++){            // Triangulating quads
