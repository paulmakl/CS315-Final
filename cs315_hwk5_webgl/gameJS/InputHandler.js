...
// keyCode translators (from http://jsfiddle.net/vWx8V/)
...
function InputHandler() {
	// need to save a reference to this because the 'this' keyword gets mangled in event callbacks
	var self = this;

	// a list of objects that want immediate updates
	this.updateObjects = [];
	...
	this.addUpdateObject = function(obj) {
		self.updateObjects.push(obj);
	};


	/*
	 * Returns true if the key specified is currently being pressed
	 */
	...
	this._keyEvent = function(evt) {
		// get a string that represents the key being pressed
		var key = keyCodeToChar[evt.which || evt.keyCode];

		// update the dict with the keys currently pressed
		if (evt.type == "keydown") {
			self.keys[key] = evt;
		}
